...
function EvalPage({ t, lang, data }) {
  const [liveData, setLiveData]   = useState(null);   // mapped live results
  const [running, setRunning]     = useState(false);
  const [runError, setRunError]   = useState(null);
  const [category, setCategory]   = useState("all");
  const pollRef                   = useRef(null);

  // Map /admin/eval/results response → UI shape
  function mapEvalResults(apiResp) {
    if (!apiResp || !apiResp.latest) return null;
    const l = apiResp.latest;
    const cat = (k) => l.by_category?.[k] ?? { total: 0, passed: 0 };
    const inFaq = cat("in_faq");
    const coll  = cat("colloquial");
    const fb    = cat("not_in_faq");
    const amb   = cat("ambiguous");

    const cases = (l.cases ?? []).map((c, i) => ({
      id:       i + 1,
      category: (c.type ?? "in_faq").replace(/_/g, "-"),
      q:        c.question ?? c.input ?? c.q ?? "—",
      expected: c.expected ?? "—",
      actual:   c.actual   ?? (c.passed ? "(pass)" : "(fail)"),
      status:   c.passed ? "pass" : "fail",
      lat:      +((c.elapsed ?? c.latency_s ?? 0)).toFixed(2),
    }));

    const history = (apiResp.history ?? []).slice(-7).map((h) => ({
      date: (h.date ?? "").slice(5),   // "2026-05-27" -> "05-27"
      pass: +(h.pass_rate ?? 0),
      lat:  +(h.avg_latency ?? 0),
    }));
    ...
    return {
      summary: {
        inFaqTotal: inFaq.total,   inFaqPass: inFaq.passed,
        colloquialTotal: coll.total, colloquialPass: coll.passed,
        fallbackTotal:   fb.total,   fallbackPass:   fb.passed,
        ambiguousTotal:  amb.total,  ambiguousPass:  amb.passed,
        avgLatency: +(l.avg_latency ?? 0).toFixed(2),
        lastRun:    l.last_run ?? "—",
        lastRunTs:  l.last_run_ts ?? 0,
        passRate:   +(l.pass_rate ?? 0),
      },
      history,
      cases,
    };
  ...
  // Fetch latest eval results from API
  const fetchLive = useCallback(async () => {
    try {
      const r = await fetch("/admin/eval/results", { cache: "no-store" });
      if (!r.ok) return null;
      return await r.json();
    } catch { return null; }
  }, []);
  ...
  // On mount: load once
  useEffect(() => {
    fetchLive().then((resp) => setLiveData(mapEvalResults(resp)));
  }, []);

  // Poll every 3s while running, stop when last_run_ts advances
  useEffect(() => {
    if (!running) { clearInterval(pollRef.current); return; }
    const prevTs = liveData?.summary?.lastRunTs ?? 0;
    pollRef.current = setInterval(async () => {
      const resp   = await fetchLive();
      const mapped = mapEvalResults(resp);
      if (mapped && mapped.summary.lastRunTs > prevTs) {
        setLiveData(mapped);
        setRunning(false);
      }
    }, 3000);
  ...
  const handleRunEval = async () => {
    ...
    setRunError(null);
    try {
      const r = await fetch("/admin/eval/run", { method: "POST", cache: "no-store" });
      if (!r.ok) { setRunError("HTTP " + r.status); setRunning(false); }
    } catch (e) { setRunError(e.message); setRunning(false); }
  };

  // Display data: live if available, otherwise empty state
  const e = liveData;
  const s = e?.summary;

  // Running banner
  const runningBanner = running && (
    <div className="pill warn" style={{ display:"inline-flex", gap:6, alignItems:"center", marginBottom:16, padding:"6px 14px", fontSize:13 }}>
      <span className="dot" style={{ background:"var(--warn)", animation:"pulse 1s infinite" }}/>
      {lang === "vi" ? "Đang chạy eval — khoảng 2–5 phút..." : "Running eval — may take 2–5 minutes..."}
    </div>
  );
    ...
    return (
      <div>
        <div className="page-head">
          ...
          <div className="page-actions">
            <button className="btn primary" onClick={handleRunEval}>
              <Icon name="play" size={13}/>{t.run_eval}
            </button>
          </div>
        </div>
        {runError && <div style={{ color:"var(--bad)", marginBottom:12, fontSize:13 }}>{runError}</div>}
        <div className="card" style={{ padding: "48px 24px", textAlign:"center", color:"var(--text-muted)", fontSize:14 }}>
          {lang === "vi"
            ? "Chua co ket qua eval. Chay 'python -m tests.evaluate --write' hoac bam 'Chay eval'."
            : "No eval results yet. Run 'python -m tests.evaluate --write' or click 'Run eval'."}
        </div>
      </div>
    );
  }
        ...
        <div className="page-head">
          ...
          <div className="page-actions">
            <button className="btn primary" disabled>
              <Icon name="refresh" size={13}/>{lang === "vi" ? "Dang chay..." : "Running..."}
            </button>
          </div>
        </div>
        {runningBanner}
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <div className="page-actions">
          <button className="btn"><Icon name="download" size={13}/>{t.export}</button>
          <button className="btn primary" onClick={handleRunEval} disabled={running}>
            <Icon name={running ? "refresh" : "play"} size={13}/>
            {running ? (lang === "vi" ? "Dang chay..." : "Running...") : t.run_eval}
          </button>
        </div>
      </div>
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">{lang === "vi" ? "% cau test pass — 7 ngay gan nhat" : "% test cases passing — last 7 runs"}</div>
            </div>
          </div>
          <div className="card-body">
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">{lang === "vi" ? "Thoi gian trung binh moi cau test (giay)" : "Average seconds per test case"}</div>
            </div>
          </div>
          <div className="card-body">
